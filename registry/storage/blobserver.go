	...
	if cachehit {
		if reqtype == "LAYER" {
			context.GetLogger(ctx).Debugf("NANNAN: layer cache hit: reqtype: %s, metadata lookup time: %v, layer cache access time: %v, "+
				...
				reqtype, DurationML, DurationMAC, DurationNTT,
				(DurationML + DurationMAC + DurationNTT),
				size)
		...
		if reqtype == "LAYER" {
			...
			context.GetLogger(ctx).Debugf("NANNAN: layer construct: reqtype: %s, %s:         metadata lookup time: %v, layer transfer and merge time: %v, "+
				"layer transfer time: %v, total time: %v, layer compressed size: %v, layer   uncompressed size: %v, compressratio: %.3f",
				reqtype, tp, DurationML, DurationLCT, DurationNTT, (DurationML + DurationLCT + DurationNTT), size, Uncompressedsize, compressratio)
